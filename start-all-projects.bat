...
REM Law Simulation (law_de) + Case File Library (dummy) + AI Contract Analysis System
...
start "Court Backend" cmd /c "cd /d "%~dp0law_de\backend" && python -m uvicorn app.main:app --host 127.0.0.1 --port 8000"
timeout /t 2 /nobreak > nul
start "Court Frontend" cmd /c "cd /d "%~dp0law_de\frontend" && npm run dev"
